...
export const PCInfo: React.FC = () => {
    ...
    const handleConfirmBulkDelete = () => {
        setPcs(prevPcs => prevPcs.filter(pc => !selectedPcIds.includes(pc.id)));
        setSelectedPcIds([]);
    ...
    const handleConfirmStatusUpdate = () => {
        setPcs(prevPcs =>
            prevPcs.map(pc =>
                selectedPcIds.includes(pc.id) ? { ...pc, status: newStatus } : pc
            )
        );
        setSelectedPcIds([]);
    ...
    const handleImportPcs = (data: Partial<PCInfoEntry>[]): { success: boolean, message: string } => {
        try {
